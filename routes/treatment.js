...
router.post('/',ensureAuthenticated, async(req,res)=>{
    ...
    const treatment = new Treatment({
        user:req.user.id,
        treatmentName: req.body.treatmentName.trim(),
    ...
    try{
        console.log(req.body.products_needed_to_do_the_treatment.split(','))
       ...
       if(req.body.products_needed_to_do_the_treatment.split(',').length > 1){
        await  req.body.products_needed_to_do_the_treatment.split(',').forEach(async(e) =>{
            ...
            if(product.length === 0){
                let productNew = new ProductsForTreatment({
                    user:req.user.id,
                    prodTreatmentName: req.body.products_needed_to_do_the_treatment.trim()
                })
                await productNew.save();
            }
       }
...
router.put('/edit/:id',ensureAuthenticated, async(req,res)=>{
    ...
    try{
        treatment = await Treatment.findById(req.params.id)
        treatment.treatmentName = req.body.treatmentName.trim()
        ...
        treatment.products_needed_to_do_the_treatment = req.body.productsForTreatment.trim().toString()
